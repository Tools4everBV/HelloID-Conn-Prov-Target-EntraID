...
try {
    ...
    if (($correlatedAccount | Measure-Object).count -eq 1) {
        ...
        if ($actionContext.Configuration.updateManagerOnUpdate -eq $true) {
            ...
            $getEntraIDAccountManagerSplatParams = @{
            ...
            Write-Verbose "SplatParams: $($getEntraIDAccountManagerSplatParams | ConvertTo-Json)"
    
            # Add header after printing splat
            $getEntraIDAccountManagerSplatParams['Headers'] = $headers
    
            try {
                $getEntraIDAccountManagerResponse = $null
                $getEntraIDAccountManagerResponse = Invoke-RestMethod @getEntraIDAccountManagerSplatParams
                $previousManagerId = $getEntraIDAccountManagerResponse.id
            }
            catch {
                $ex = $PSItem
                ...
                if ($auditMessage -like "*Resource 'manager' does not exist*") {
            ...
            Write-Verbose "Queried manager of account with AccountReference: $($actionContext.References.Account | ConvertTo-Json). Result: $($getEntraIDAccountManagerResponse | ConvertTo-Json)"
    ...
    if ($actionContext.Configuration.updateManagerOnUpdate -eq $true) {      
        switch ($actionManager) {
            "Update" {
                ...
                $actionMessage = "updating manager for account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)"
                ...
                if (-Not($actionContext.DryRun -eq $true)) {
                    ...
                    $outputContext.AuditLogs.Add([PSCustomObject]@{
                            # Action  = "" # Optional
                            Message = "Updated manager for account with AccountReference: $($actionContext.References.Account | ConvertTo-Json). Old value: $($previousManagerId | ConvertTo-Json). New value: $($actionContext.References.ManagerAccount | ConvertTo-Json)."
                            IsError = $false
                        })
                }
                else {
                    Write-Warning "DryRun: Would update manager for account with AccountReference: $($actionContext.References.Account | ConvertTo-Json). Old value: $($previousManagerId | ConvertTo-Json). New value: $($actionContext.References.ManagerAccount | ConvertTo-Json)."
                }
            ...
            "NoChanges" {
                #region No changes
                $actionMessage = "skipping updating manager for account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)"
                ...
                $outputContext.AuditLogs.Add([PSCustomObject]@{
                        # Action  = "" # Optional
                        Message = "Skipped updating manager for account with AccountReference: $($actionContext.References.Account | ConvertTo-Json). Reason: No changes."
                        IsError = $false
                    })
                #endregion No changes
            ...
            "Clear" {
                ...
                # API docs: https://learn.microsoft.com/en-us/graph/api/user-delete-manager?view=graph-rest-1.0&tabs=http
                $actionMessage = "clearing manager for account with AccountReference: $($actionContext.References.Account | ConvertTo-Json)"
                ...
                if (-Not($actionContext.DryRun -eq $true)) {
                    ...
                    #region Set ManagerId
                    $outputContext.Data | Add-Member -MemberType NoteProperty -Name "managerId" -Value $null -Force
                    #endregion Set ManagerId

                    $outputContext.AuditLogs.Add([PSCustomObject]@{
                            # Action  = "" # Optional
                            Message = "Cleared manager for account with AccountReference: $($actionContext.References.Account | ConvertTo-Json). Old value: $($previousManagerId | ConvertTo-Json)."
                            IsError = $false
                        })
                }
                else {
                    Write-Warning "DryRun: Would clear manager for account with AccountReference: $($actionContext.References.Account | ConvertTo-Json). Old value: $($previousManagerId | ConvertTo-Json)."
                }
